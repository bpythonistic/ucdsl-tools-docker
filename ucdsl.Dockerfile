...
RUN apt-get install autoconf ocaml ocaml-native-compilers camlp4-extra opam libgmp-dev libpcre2-dev pkg-config zlib1g-dev -y
...
# Setup UC-DSL
RUN cd ~ && git clone https://github.com/easyuc/EasyUC.git && cd EasyUC/uc-dsl/ && eval $(opam env) && echo "/home/primary/.opam/5.3.0/lib/easycrypt/theories" | ./configure && ./build && ./install-opam 
