...
RUN apt-get install autoconf ocaml ocaml-native-compilers camlp4-extra opam libgmp-dev libpcre3-dev pkg-config zlib1g-dev emacs -y
...
# Setup UC-DSL
RUN cd ~ && git clone https://github.com/easyuc/EasyUC.git && cd EasyUC/uc-dsl/ && eval $(opam env) && echo "/home/headless/.opam/5.1.1/lib/easycrypt/theories" | ./configure && ./build && ./install-opam
